...
try {
    ...
    console.log(
        `🌌 Đã tải ${LINH_CAN.length} Linh Căn.`
    );

} catch (error) {

    console.error(
        "❌ Không tải được linhcan.js:"
    );
...
try {
    ...
    THE_CHAT =
        theChatData.THE_CHAT || [];

    console.log(
        `🧬 Đã tải ${THE_CHAT.length} Thể Chất.`
    );

} catch (error) {

    console.error(
        "❌ Không tải được thechat.js:"
    );
...
client.once(
    ...
    () => {
        ...
        console.log(
            `🌌 Linh Căn: ${LINH_CAN.length}`
        );

        console.log(
            `🧬 Thể Chất: ${THE_CHAT.length}`
        );
